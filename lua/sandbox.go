...
func SetupSandbox(L *lua.LState, db *storage.DB) {
	...
	// math API (gonum-powered)
	mathTable := L.NewTable()
	L.SetField(mathTable, "mat_new", L.NewFunction(mathMatNew(db)))
	...
	L.SetField(mathTable, "tensor_new", L.NewFunction(mathTensorNew(db)))
